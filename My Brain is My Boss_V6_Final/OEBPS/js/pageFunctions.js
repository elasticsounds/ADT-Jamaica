...
function play_audio_h1(audio, high, audio1) {
    ...
    if (audio === 'answer_incorrect') {
        audio = 'hat_incorrect_' + (Math.floor(Math.random() * 2) + 1)
    }

    if (audio === 'hat_correct') {
        audio = 'hat_correct_' + (Math.floor(Math.random() * 4) + 1)
    }
...
function play_audio_atri(audio) {
    /* var sounds = document.getElementsByTagName('audio');
     for (i = 0; i < sounds.length; i++) {
     sounds[i].pause();
     sounds[i].currentTime = 0.1;
